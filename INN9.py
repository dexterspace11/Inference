...
if cluster_file and missing_file and output_path:
    try:
        ...
        # Load the original scaler from the CNN-EQIC Excel
        scaler = load_scaler_from_excel(cluster_file)

        # Inverse transform the inferred scaled values to original scale
        scaled_values = inferred_data[target_columns].values
        unscaled_values = scaler.inverse_transform(scaled_values)

        # Add inverse-transformed columns
        for i, col in enumerate(target_columns):
            inferred_data[col + "_unscaled"] = unscaled_values[:, i]

        st.subheader("🧩 Inverse-Transformed Values (Original Scale)")
