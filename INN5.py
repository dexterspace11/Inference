# ---------------- Full CNN-EQIC + INN Streamlit App (Fixed Missing Variable Handling) ----------------
...
with tabs[1]:
    ...
    if cnn_eqic_file and raw_test_file and output_path:
        try:
            ...
            # Automatically detect missing variable to infer (must be among missing_features)
            if missing_features:
            ...
            if missing_required:
                st.error(f"\u274c Missing required variables in test data: {missing_required}")
                st.stop()

            # IMPORTANT FIX: Only reorder test df columns with those that exist (exclude missing_var which is missing)
            df_test = df_test[required_inputs]

            df_test_lagged = create_lagged_features(df_test, base_features, window_size)

            # Prepare columns to use as inputs (exclude missing_var lags)
            used_cols = [col for col in df_centroids.columns if not col.startswith(missing_var + "_t")]
            # Columns representing the target variable lags (to infer)
            target_cols = [col for col in df_centroids.columns if col.startswith(missing_var + "_t")]

            inferred_data = df_test_lagged.copy()

            # Fit a linear regression model per lag target and predict missing variable lags
