...
with tab[0]:
    ...
    if uploaded_file:
        ...
        if not numerical_cols:
            st.error("No numeric columns detected in uploaded data.")
            st.stop()

        default_selection = numerical_cols[:min(4, len(numerical_cols))]
        selected = st.multiselect("Select features for clustering", numerical_cols, default=default_selection, key="cnn_features")

        window_size = st.slider("Window size (time steps)", 2, 20, 5, key="cnn_window")
        ...
        if len(selected) >= 2:
            ...
            # Build patterns using sliding windows
            ...
            # Collect episodic memory patterns
            patterns = [p for e in net.episodic_memory.episodes.values() for p in e['patterns']]
            if len(patterns) == 0:
                st.warning("No patterns generated by neural network. Try adjusting parameters or input data.")
                st.stop()
            patterns = np.array(patterns)

            expanded_feature_names = [f"{feat}_t{t}" for t in range(window_size) for feat in selected]

            k_max = min(10, len(patterns))
            k = st.slider("Number of clusters (k)", 2, k_max, 5, key="cnn_clusters")
...
with tab[1]:
    ...
    if cnn_eqic_file and raw_test_file and output_path:
        try:
            ...
            if raw_test_file.name.endswith(".csv"):
                df_test = pd.read_csv(raw_test_file)
            else:
                df_test = pd.read_excel(raw_test_file)
